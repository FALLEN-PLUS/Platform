...
(function ()
{
    ...
    class SelectView
    {
        constructor(select)
        {
            ...
            if (select.classList.contains("select-overlay")) this.wrapper.classList.add("mapping");
            if (select.classList.contains("select-mini") || select.classList.contains("select-send-opt")) this.wrapper.classList.add("compact");
    ...
    function init()
    {
        document.querySelectorAll("select:not(#frameList)").forEach(select => { if (!instances.has(select)) instances.set(select, new SelectView(select)); });
    }
